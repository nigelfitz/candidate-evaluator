...
if current_page == "Upload & Analyse":
    ...
    if current_jd and current_candidates:
        ...
        with insight_col1:
            ...
            if insight_mode == "Top N candidates":
                top_n = st.slider(
                    ...
                    help="Candidates will be ranked by overall score, and insights generated for the top N"
                )
                ...
                selected = st.multiselect(
                    ...
                    help="Choose which candidates should receive GPT insights"
                )
        ...
        with insight_col2:
            # Cost impact display
            num_insights = st.session_state.get("insights_top_n", 5)
            total_candidates = len(current_candidates)
            
            st.metric("Candidates with insights", f"{num_insights}/{total_candidates}")
            st.caption(f"Scoring: All {total_candidates}")
            
            if num_insights < total_candidates:
                st.success(f"💰 Reduced cost: ~{((total_candidates - num_insights) / total_candidates * 100):.0f}% savings")
    ...
    with st.sidebar:
        ...
        if current_jd and current_candidates:
            ...
            st.caption(f"Insights for {num_insights} of {len(current_candidates)} candidates")
            
            # Expandable breakdown
            with st.expander("💡 Details"):
                st.write(f"**JD Extraction:** ${cost_info['breakdown']['jd_extraction']:.2f}")
                st.write(f"**Insights ({num_insights} candidates):** ${cost_info['breakdown']['candidate_insights']:.2f}")
                st.caption(f"Model: {cost_info['model']}")
                st.caption(f"Scoring is free for all {len(current_candidates)} candidates")
