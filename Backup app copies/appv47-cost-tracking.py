...
if current_page == "Upload & Analyse":
    ...
    if current_jd and current_candidates:
        st.markdown("---")
        st.markdown("### 💰 Estimated Analysis Cost")
        
        # Get criteria count (estimate if not yet generated)
        criteria_text = st.session_state.get("criteria_text", "")
        if criteria_text:
            num_criteria = len([c for c in criteria_text.splitlines() if c.strip()])
        else:
            # Estimate ~20 criteria from JD
            num_criteria = 20
        ...
        with col1:
            st.metric("Total Estimated Cost", f"${cost_info['estimated_cost_usd']:.2f}")
        with col2:
            st.metric("Cost per Candidate", f"${cost_info['cost_per_candidate']:.2f}")
        with col3:
            st.metric("Total Candidates", len(current_candidates))
        
        with st.expander("💡 Cost Breakdown"):
            st.write(f"**JD Extraction:** ${cost_info['breakdown']['jd_extraction']:.2f}")
            st.write(f"**Candidate Insights:** ${cost_info['breakdown']['candidate_insights']:.2f}")
            st.caption(f"Using model: {cost_info['model']}")
            st.caption("Note: This is an estimate. Actual costs may vary slightly based on exact token usage.")
...
# PDF Tools page removed - functionality moved to Help & FAQ > PDF Text Extraction section
