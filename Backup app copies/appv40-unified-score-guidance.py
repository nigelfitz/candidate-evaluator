...
if current_page == "Upload & Analyse":
    ...
    if isinstance(covdf, pd.DataFrame) and not covdf.empty:
        ...
        with result_col1:
            ...
            st.caption("*Scores show match strength to JD (0.00–1.00; higher = better)*")
            try:
                ...
                for idx, r in top3.iterrows():
                    ...
                    st.write(f"**{idx+1}.** {r['Candidate']} — {score:.2f} {icon}")
            except Exception:
                pass
    ...
    with header_col1:
        ...
        if not isinstance(coverage_df, pd.DataFrame) or coverage_df.empty:
            ...
            # Info box with scoring guide - now includes numeric thresholds
            st.info(
                "**Score Guide:** Each score (0.00–1.00) measures how well a candidate's resume matches a criterion. "
                "Colors indicate strength: ✅ Strong (≥0.70) | ⚠️ Moderate (0.45–0.70) | ⛔ Weak (<0.45)", 
                icon="ℹ️"
            )
    ...
    with st.expander("📈 Detailed Scores by Criterion", expanded=False):
        st.markdown(
            "View individual criterion scores with color-coding "
