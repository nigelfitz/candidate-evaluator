...
with st.sidebar:
    ...
    pages = ["Upload & Analyse", "JD & Criteria", "Coverage", "Insights", "Compare", "Candidates", "Scoring Explained", "Export", "PDF Tools", "Settings"]
...
if current_page == "Upload & Analyse":
...
elif current_page == "JD & Criteria":
    st.markdown("<div style='margin-top: 1.5rem;'></div>", unsafe_allow_html=True)
    st.subheader("Job Description (JD) & Criteria")
    if st.session_state.get("jd"):
    ...
    with ctrl_col1:
        sort_by = st.selectbox("Sort by", 
                              ["Name (A-Z)", "Overall Score (High-Low)", "Overall Score (Low-High)"],
                              index=st.session_state.get("cov_sort_idx", 1),
                              key="sort_select",
                              label_visibility="visible")
        st.session_state.cov_sort_idx = ["Name (A-Z)", "Overall Score (High-Low)", "Overall Score (Low-High)"].index(sort_by)
    ...
    with top_candidates_placeholder.container():
        if isinstance(coverage_df, pd.DataFrame) and not coverage_df.empty:
            ...
            for idx, row in top_5_df.iterrows():
                ...
                if show_scores:
                    st.markdown(f"**{pos}.** {row['Candidate']} — {icon} {score:.2f}")
                else:
    ...
    avg_by_criterion = {}
    for crit in ordered:
        scores = [float(mat.loc[crit, cand]) for cand in candidates]
    ...
    st.markdown("*Top 5 criteria:*")
    for crit, score in top_5:
        st.write(f"• {crit}: {score:.2f}")
    ...
    if crit and cand_choice:
        ...
        if snip is None:
            ...
            score_color = "#28a745" if sc >= hi else ("#ffc107" if sc >= lo else "#dc3545")
            st.markdown(f"**Score:** <span style='color: {score_color}; font-size: 1.2rem; font-weight: bold;'>{sc:.2f}</span>", unsafe_allow_html=True)
