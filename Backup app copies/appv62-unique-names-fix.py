...
if current_page == "Upload & Analyse":
    ...
    # Make candidate names unique for display (append #1, #2, etc. to duplicates)
    unique_candidates = []
    name_counts = {}
    for name in candidates:
        if name in name_counts:
            name_counts[name] += 1
            unique_candidates.append(f"{name} #{name_counts[name]}")
        else:
            name_counts[name] = 1
    ...
    disp = pd.DataFrame(
        ...
        columns=["Average"] + unique_candidates
    )
    ...
    for idx, cand in enumerate(candidates):
        ...
        disp.loc[("📊 Overall", "Overall Score"), unique_candidates[idx]] = f"{overall:.2f} {icon}"  # Always show score (score first for proper sorting)
    ...
    for crit in ordered:
        ...
        for cand in candidates:
            ...
            # Handle case where mat.loc returns a Series (duplicate names)
        ...
        for idx, c in enumerate(candidates):
            ...
            disp.loc[(cat_map.get(crit,"uncategorised"), crit), unique_candidates[idx]] = f"{val:.2f} {icon}" if show_scores else icon
