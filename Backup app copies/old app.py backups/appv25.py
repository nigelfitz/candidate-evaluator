...
# Tabs
overview_tab, jd_tab, coverage_tab, insights_tab, compare_tab, candidates_tab, scoring_tab, export_tab, pdf_tab, settings_tab = st.tabs(["Overview","Job Description","Coverage","Insights","Compare","Candidates","Scoring Explained","Export","PDF Tools","Settings"])
...
with jd_tab:
    ...
    # --- Analysis limit control ---
    st.markdown("#### Analysis limit")
    _default_limit = int(st.session_state.get("criteria_limit", 30))
    _limit = int(st.number_input("Max criteria used in analysis", min_value=1, max_value=10000, value=_default_limit))
    if _limit != _default_limit:
        st.session_state["criteria_limit"] = _limit
    ...
    import pandas as pd
    rows = []
    _limit = int(st.session_state.get("criteria_limit", 30))
    for _i, c in enumerate(init_lines):
        _use = _i < _limit
        rows.append({"Use": _use, "Criterion": c, "Category": cat_map.get(c, "")})
    if not rows:
...
# Insight Engine selection (default gpt-4o-mini)
    ...
    labels = [
        "gpt-4o-mini — fast, cost-efficient; great plain-English insights",
        "gpt-4o — highest-quality phrasing and reasoning; slightly slower",
    ]
    engines = ["gpt-4o-mini", "gpt-4o"]
    current_engine = st.session_state.get("insight_engine", "gpt-4o-mini")
    default_idx = engines.index(current_engine) if current_engine in engines else 0
    choice_label = st.radio(
        "Generate candidate insights using:", labels, index=default_idx,
        help="Choose the OpenAI model for narrative insights."
    )
    st.session_state["insight_engine"] = engines[labels.index(choice_label)]
