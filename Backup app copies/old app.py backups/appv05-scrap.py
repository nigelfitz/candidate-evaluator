...
def chunk_text(text: str, target: int = 9000, overlap: int = 400) -> list[str]:
    ...
    for p in paras:
        if not buf:
            ...
            chunks.append(buf)
            # start next buffer with small overlap from end of previous
...
def merge_candidate_chunks(datas: list[dict]) -> dict:
    """
    Merge per-chunk candidate JSONs: concatenate arrays, prefer first non-empty scalar.
    """
    out = {
