...
def extract_text_from_pdf_robust(file_bytes: bytes) -> str:
    """
    ...
    if len((text or "").strip()) < 200:  # low yield, consider OCR
...
def gpt_candidate_insights(
    ...
    for c, sc in top_items:
        ...
        if redacted:
            snip = _redact(snip)
        ev_lines.append(f"- {c} (score {sc:.2f}): {snip[:400].replace('\n',' ')}")
...
with st.sidebar:
    ...
    use_robust = st.checkbox("Use robust PDF extractor (PyMuPDF + OCR fallback)", value=False,
                             help="If your JD is a scanned PDF or pdfplumber extracts little text, enable this.")
    ...
    if jd_file is not None:
        b = jd_file.read()
        jd_text_plumber = read_file_bytes(b, jd_file.name)
        jd_text = jd_text_plumber
        if use_robust and jd_file.name.lower().endswith(".pdf"):
            if fitz is None:
                st.warning("PyMuPDF not installed; robust extraction unavailable.", icon="⚠️")
            else:
                jd_text = extract_text_from_pdf_robust(b) or jd_text_plumber
...
# Tabs
overview_tab, jd_tab, coverage_tab, insights_tab, compare_tab, labels_tab, scoring_tab, export_tab, settings_tab = st.tabs(["Overview","Job Description","Coverage","Insights","Compare","Candidate Labels","Scoring Explained","Export","Settings"])
