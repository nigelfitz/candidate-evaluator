...
def call_gpt(messages, model: str = "gpt-4o-mini", temperature: float = 0.1) -> str:
    """
    Chat completion using modern OpenAI client.
    """
...
# -------------------------------
# Generic normaliser + richer heuristic scorer (no embeddings)
# -------------------------------
...
def score_candidate_semantic(candidate: dict, jd: dict, threshold: float, w_exact: float, w_token: float, w_sem: float) -> dict:
    ...
    Final = normalized weights (w_exact, w_token, w_sem).
    """
...
def chunk_text(text: str, target: int = 9000, overlap: int = 400) -> List[str]:
    """
    Split text into ~target-char chunks on paragraph/line boundaries with small overlap.
    """
    text = text or ""
    if len(text) <= target:
        return [text]
...
with st.sidebar:
    ...
    # Weight sliders (normalized internally)
...
def _approx_tokens(chars: int) -> int:
    return max(1, int(round(chars / 4)))  # rough heuristic
...
with tabs[0]:
    ...
    estimated_costs = {}    # store cost estimations
    ...
    if run_bulk:
        if jd_file is None:
            ...
            if len(jd_text.strip()) < 50:
                ...
                # Cost: JD text to chat (per chunk)
                ...
                with st.spinner(f"Analysing {len(cv_files)} candidate(s)…"):
                    for f in cv_files:
                        ...
                        # Candidate extraction (chunked or not)
                ...
                if analysis_rows:
                    ...
                    # Cost meter (if any prices entered)
                ...
                # Keep JD + candidate packets in session for Developer tab
