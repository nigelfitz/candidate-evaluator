...
def generate_candidate_insight(cand: dict, jd: dict, scores: dict, model: str = "gpt-4o-mini") -> str:
    ...
    payload = {"candidate": cand or {}, "job": jd or {}, "scores": scores or {}}
    user = (
...
with st.sidebar:
    with st.expander("⚙️ Settings", expanded=False):
        ...
        model = st.selectbox(
            ...
            ["gpt-4o-mini", "gpt-4o"],
            index=0,
            help="Model used for parsing CVs/JDs and generating insights."
        )
        ...
        USE_SEMANTIC = st.checkbox(
            "Use semantic matching (embeddings)",
            value=True,
            help="Enable meaning-based matches so synonyms/abbreviations count (e.g., FP&A ≈ Financial Planning & Analysis)."
        )
        SEM_THRESHOLD = st.slider(
            "Semantic match threshold",
            0.70, 0.95, 0.82, 0.01,
            help="Higher = stricter. A JD skill only counts semantically if similarity ≥ this threshold."
        )
        ...
        W_EXACT = st.slider(
            "Weight: exact",
            0.0, 1.0, 0.60, 0.05,
            help="Exact phrase overlap (strict keyword match)."
        )
        W_TOKEN = st.slider(
            "Weight: token",
            0.0, 1.0, 0.15, 0.05,
            help="Word-level overlap across phrases (partial credit when wording differs)."
        )
        W_SEM   = st.slider(
            "Weight: semantic",
            0.0, 1.0, 0.25, 0.05,
            help="Meaning-based coverage using embeddings (finds related phrases)."
        )
        ...
        USE_CHUNKING = st.checkbox(
            "Use chunking for long CV/JD",
            value=True,
            help="Split long texts into chunks so the model reads everything reliably."
        )
        CHUNK_TARGET = st.slider(
            "Chunk size (chars)",
            6000, 12000, 9000, 500,
            help="Target chunk size. Overlap is handled automatically."
        )

        st.divider()
        st.subheader("Insights")
        MAKE_INSIGHTS = st.checkbox(
            "Generate candidate insight panels (uses GPT)",
            value=True,
            help="Short recruiter-style summary for each candidate (adds model calls)."
        )

        st.divider()
        st.subheader("Cost meter (optional)")
        st.caption("We estimate ~4 chars/token. Enter your prices (per 1K tokens).")
        PRICE_CHAT_IN  = st.number_input(
            "Price: chat input ($/1K tok)", 0.0, 10.0, 0.0, 0.01,
            help="Cost for text sent to the chat model (inputs only, estimate)."
        )
        PRICE_EMB_IN   = st.number_input(
            "Price: embedding input ($/1K tok)", 0.0, 10.0, 0.0, 0.01,
            help="Cost for text sent to the embeddings model (inputs only, estimate)."
        )

        # --- In-sidebar Scoring guide ---
        with st.expander("ℹ️ Scoring guide", expanded=False):
            st.markdown(f"""
**Heuristic score**  
Uses two parts:  
• **Exact** — whole phrase overlaps (strict keywords)  
• **Token** — word-level overlap (partial credit when phrasing differs)

**Semantic score**  
Adds meaning-based matches using embeddings (cosine similarity).  
• A JD skill **counts** when similarity ≥ **threshold = {SEM_THRESHOLD}**  
• Final semantic score blends:  
  - Exact (**{W_EXACT:.2f}**), Token (**{W_TOKEN:.2f}**), Semantic (**{W_SEM:.2f}**) — normalized to sum to 1

**When to use what**  
• Prefer **heuristic** for must-have credentials (“CPA”, “CISSP”, “C++”).  
• Turn on **semantic** for broader capability areas (“stakeholder management”, “process improvement”).  
• If false positives appear: raise the threshold or lower the semantic weight.  
""")

# Add Help tab
tabs = st.tabs(["Analysis", "Developer", "Help"])
        ...
        if jd_file is None:
            ...
            if len(jd_text.strip()) < 50:
                ...
                if USE_CHUNKING and len(jd_text) > MAX_CHARS:
                    chunks = chunk_text(jd_text, target=9000)
                    per = []
                ...
                with st.spinner(f"Analysing {len(cv_files)} candidate(s)…"):
                    for f in cv_files:
                        ...
                        if USE_CHUNKING and len(cv_text) > MAX_CHARS:
                            chunks = chunk_text(cv_text, target=9000)
                            per = []
                            for ch in chunks:
                                _add_cost(estimated_costs, "chat_input_$", _approx_tokens(len(ch)), PRICE_CHAT_IN)
                        ...
                        candidate_packets.append({
                            ...
                            "score_heur": s_heur,
                            "score_sem": s_sem,
                            # "insight": will be populated below if enabled
                        })
                ...
                if analysis_rows:
                    ...
                    # Scoring legend expander (above table)
...
**Semantic score** → adds **meaning-based** matching using OpenAI embeddings.  
- Finds similar/related phrases even when worded differently.  
- Controlled by **semantic threshold** = `{SEM_THRESHOLD}`.  
- Weighted combination of:
  - Exact: **{W_EXACT:.2f}**
  - Token: **{W_TOKEN:.2f}**
  - Semantic: **{W_SEM:.2f}**  (normalized to sum to 1)
""")
                ...
                if jd_data and analysis_rows:
                    ...
                    if not criteria:
                        ...
                        # Header with inline Help toggle (works inside tabs)
                        if "show_cov_help" not in st.session_state:
                            st.session_state["show_cov_help"] = False

                        hcol1, hcol2 = st.columns([1, 0.12])
                        with hcol1:
                            st.subheader("Criteria coverage matrix")
                        with hcol2:
                            if st.button("❓ Help", key="cov_help_toggle"):
                                st.session_state["show_cov_help"] = not st.session_state["show_cov_help"]

                        if st.session_state["show_cov_help"]:
                            st.info(
                                "✅ **Meets** – exact phrase or semantic match, or strong token overlap.\n\n"
                                "⚠️ **Partial** – some token overlap but no exact/semantic hit.\n\n"
                                "⛔ **Missing** – little or no overlap.\n\n"
                                "Use **Evidence explorer** below to view text snippets from each CV that support these matches. "
                                "Semantic matches depend on your threshold and weight settings (see the Settings panel)."
                            )
                        ...
                        # Build DataFrame and color style (robust to missing columns)
                        mat_df = pd.DataFrame(matrix_rows)
                        # Ensure ALL candidate columns exist (fill missing with ⛔)
                        mat_df = mat_df.reindex(columns=["Criterion"] + cand_files, fill_value="⛔")

                        display_map = {r["File"]: r["Candidate"] for r in analysis_rows}
                        color_map = {"✅": "#e6ffe6", "⚠️": "#fff8e6", "⛔": "#ffe6e6"}

                        # Style only the data columns actually present (exclude "Criterion")
                        data_cols = [c for c in mat_df.columns if c != "Criterion"]

                        def _colorize_dataframe(subdf: pd.DataFrame) -> pd.DataFrame:
                            return subdf.applymap(lambda v: f"background-color: {color_map.get(v, '')};" if v in color_map else "")

                        styler = mat_df.style
                        if data_cols:
                            idx = pd.IndexSlice
                            styler = styler.apply(_colorize_dataframe, axis=None, subset=idx[:, data_cols])

                        # Prepare display copy with reader-friendly headers
                        display_cols = ["Criterion"] + [display_map.get(c, c) for c in cand_files]
                        mat_df_display = mat_df.copy()
                        mat_df_display.columns = display_cols

                        # Rebuild styler on the display copy and apply styles to renamed data columns
                        styler = mat_df_display.style
                        if data_cols:
                            disp_data_cols = [display_map.get(c, c) for c in data_cols]
                            idx = pd.IndexSlice
                            styler = styler.apply(_colorize_dataframe, axis=None, subset=idx[:, disp_data_cols])
                ...
                # ----- Candidate Insight Panels (GENERATE & CACHE) -----
                if candidate_packets:
                    ...
                    for pkt in candidate_packets:
                        ...
                        # Generate and cache on first run if opted-in
                        if MAKE_INSIGHTS and not pkt.get("insight"):
                            ...
                            try:
                                pkt["insight"] = generate_candidate_insight(cand, jd_data, scores_for_llm, model=model)
                            except Exception as e:
                                pkt["insight"] = f"(Could not generate insight: {e})"

                        # Render if present
                        if pkt.get("insight"):
                            with st.expander(f"🧭 {name}", expanded=False):
                                st.write(pkt["insight"])

                # --- Persist all display data so later reruns can re-render without pressing "Analyse" again
                st.session_state["analysis_rows"]   = analysis_rows
                st.session_state["jd_data"]         = jd_data
                st.session_state["estimated_costs"] = estimated_costs
                if analysis_rows:
                    st.session_state["ranking_df"] = df
                if jd_data:
                    st.session_state["criteria"] = criteria
                    st.session_state["detailed_records"] = detailed_records

                # Save for Developer tab and persisted insights
    ...
    # ===========================
    # Re-render last results if available (when not pressing Analyse)
    # ===========================
    if not run_bulk and "ranking_df" in st.session_state:
        jd_data          = st.session_state.get("jd_data", {})
        analysis_rows    = st.session_state.get("analysis_rows", [])
        estimated_costs  = st.session_state.get("estimated_costs", {})
        df               = st.session_state.get("ranking_df")
        criteria         = st.session_state.get("criteria", [])
        detailed_records = st.session_state.get("detailed_records", [])
        candidate_packets= st.session_state.get("candidate_packets", [])

        # ----- Dashboard summary -----
        st.subheader("Dashboard")
        st.markdown(f"- **JD criteria detected:** {len(criteria) if criteria else 0}")
...
""")
            ...
            if (PRICE_CHAT_IN > 0 or PRICE_EMB_IN > 0) and estimated_costs:
                st.subheader("Estimated cost (inputs only)")
                pretty = {k: f"${v:0.4f}" for k, v in estimated_costs.items()}
                st.json(pretty)

        # ----- Field completeness + coverage matrix -----
        if jd_data and analysis_rows and criteria:
            st.subheader("JD field completeness")
            comp = jd_field_completeness(jd_data)
            st.markdown(f"**Completeness:** {comp['_completeness_pct']}% "
                        f"({comp['_populated_fields']}/{comp['_total_fields']} populated)")

            # Header with inline Help toggle (works in tabs)
            if "show_cov_help" not in st.session_state:
                st.session_state["show_cov_help"] = False
            ...
            with hcol1:
                st.subheader("Criteria coverage matrix")
            with hcol2:
                if st.button("❓ Help", key="cov_help_toggle_persist"):
                    st.session_state["show_cov_help"] = not st.session_state["show_cov_help"]
            ...
            # Rebuild matrix table from persisted artifacts
            rows = []
            cand_files = [row["File"] for row in analysis_rows]
            for c in criteria:
                row = {"Criterion": f"{c['section']}: {c['text']}"}
                for cf in cand_files:
                    rec = next((d for d in detailed_records
                                if d["candidate_file"] == cf
                                and d["criterion_section"] == c["section"]
                                and d["criterion_text"] == c["text"]), None)
                    row[cf] = rec["icon"] if rec else "⛔"
                rows.append(row)
            ...
            if not mat_df.empty:
                ...
                # Prepare display copy with nice headers
                display_cols = ["Criterion"] + [display_map.get(c, c) for c in cand_files]
                mat_df_display = mat_df.copy()
                mat_df_display.columns = display_cols
            ...
            # Evidence explorer
            st.subheader("Evidence explorer")
            crit_opts = [f"{c['section']}: {c['text']}" for c in criteria]
            display_map = {r["File"]: r["Candidate"] for r in analysis_rows}
            cand_opts = [display_map.get(c, c) for c in cand_files]
            colA, colB = st.columns(2)
            with colA:
                pick_crit = st.selectbox("Pick a JD criterion", crit_opts, index=0, key="persist_pick_crit")
            with colB:
                pick_cand_disp = st.selectbox("Pick a candidate", cand_opts, index=0, key="persist_pick_cand")
        ...
        # ----- Candidate Insight Panels (PERSISTED RERENDER) -----
        ins_pkts = [p for p in candidate_packets if p.get("insight")]
        if ins_pkts:
            st.subheader("Candidate insights")
            for pkt in ins_pkts:
                cand = (pkt.get("cand_res") or {}).get("data") or {}
                name = cand.get("name") or os.path.splitext(pkt["file_name"])[0]
                with st.expander(f"🧭 {name}", expanded=False):
                    st.write(pkt["insight"])
        elif MAKE_INSIGHTS and candidate_packets:
...
with tabs[2]:
    ...
    st.markdown("""
### Scores at a glance
- **Exact**: strict phrase overlap (great for hard requirements).
- **Token**: word-level overlap across phrases (partial credit).
- **Semantic**: meaning-based matches using embeddings (captures synonyms/paraphrases).

### Tuning tips
- **Threshold**: higher = stricter semantic matches. Start around **0.82**.
- **Weights**: adjust how much each component influences the final score.
  - Try **Exact 0.60 / Token 0.15 / Semantic 0.25** as a sensible default.

### Long documents
Enable **chunking** so the model reads 100% of the text. Default size works well.

### Insights
Turn on **Candidate insight panels** to generate short recruiter-style summaries.
""")

    st.markdown('<a id="help-coverage-matrix"></a>', unsafe_allow_html=True)
    st.subheader("Coverage matrix")
    st.markdown("""
- **✅ Meets**: exact phrase or semantic hit, or strong token coverage.
- **⚠️ Partial**: some token overlap, but not strong enough and no exact/semantic hit.
- **⛔ Missing**: little or no overlap detected.
Use the **Evidence explorer** to see a snippet from the CV that led to the decision.
""")
