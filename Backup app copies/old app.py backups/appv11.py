# Candidate Pack Summariser — pt3 (Coverage Matrix rev)
# Adds: transposed Coverage Matrix (criteria rows x candidates columns) with icons,
# JD-derived criterion categories, and an "Evidence Explorer" panel.
...
def load_embedder() -> Dict[str, Any]:
    # Prefer SBERT; fallback to TF-IDF/BoW
...
def prepare_corpus_embeddings(corpus_chunks: List[str], _embedder_info: Dict[str, Any]):
    """
    Returns (chunk_embeddings, vectorizer_or_meta). Uses SBERT if available,
    otherwise TF-IDF, and finally a simple bag-of-words fallback.
    """
    info = _embedder_info
...
# ---- Extract JD requirements + categories ----
CATEGORY_ALIASES = {
...
def extract_requirements_from_jd(jd_text: str) -> List[str]:
    lines = [l.rstrip() for l in (jd_text or "").splitlines()]
    bullets = []
    for l in lines:
        if re.match(r"^[-•·▪◦*]+\s+", l.strip()):
            bullets.append(re.sub(r"^[-•·▪◦*]+\s+", "", l.strip()))
        elif re.search(r"(experience|responsibil|require|skills|proven|ability|knowledge)", l.lower()) and len(l) > 25:
            bullets.append(l.strip())
    uniq, seen = [], set()
    for b in bullets:
        b = normalize_ws(b)
        if b and b.lower() not in seen:
            uniq.append(b); seen.add(b.lower())
    return uniq[:200]

def extract_categories_map(jd_text: str, criteria: List[str]) -> Dict[str, str]:
    """
    Heuristically map each criterion to a JD section heading like 'Responsibilities', 'Key Skills', etc.
    Approach: scan JD for headings (non-bullet lines ending with ':' or short title case line),
    track current section while walking forward; assign bullets beneath to that section.
    Fallback: try keyword match against heading text; default 'uncategorised'.
    """
    crit_set = {normalize_ws(c).lower(): c for c in criteria}
    mapping: Dict[str, str] = {}
    cur_section = "uncategorised"

    heading_candidates = set(CATEGORY_ALIASES.values())

    def normalise_heading(h: str) -> str:
        h0 = normalize_ws(re.sub(r":$", "", h)).lower()
        # map via aliases
        for k, v in CATEGORY_ALIASES.items():
            if k in h0:
                return v
        return h0.replace(" ", "_")[:30] or "uncategorised"
    ...
    for raw in lines:
        ...
        if not s:
            continue
        if re.match(r"^[A-Za-z].{0,60}:$", s) or (len(s) <= 40 and not re.match(r"^[-•·▪◦*]+\s+", s) and s == s.title()):
            cur_section = normalise_heading(s)
            continue
        m = re.match(r"^[-•·▪◦*]+\s+(.*)$", s)
        if m:
            bullet = normalize_ws(m.group(1)).lower()
            # Assign bullets to criteria if they match by substring overlap
            for c_lo, c_orig in crit_set.items():
                if c_lo in bullet or bullet in c_lo:
                    mapping[c_orig] = cur_section
        else:
            # paragraph line: attempt loose match
            para = normalize_ws(s).lower()
            for c_lo, c_orig in crit_set.items():
                if c_lo in para:
                    mapping[c_orig] = cur_section

    # Fallback any missing to 'uncategorised'
    for c in criteria:
        mapping.setdefault(c, "uncategorised")
    return mapping

def build_icon(score: float, hi: float, lo: float, show_score: bool) -> str:
    if score >= hi:
        s = "✅"
    elif score >= lo:
        s = "⚠️"
        ...
        s = "⛔"
    return f"{s} {score:.2f}" if show_score else s

def build_coverage_matrix_view(coverage_df: pd.DataFrame, jd_text: str, show_scores: bool, hi: float, lo: float) -> Tuple[pd.DataFrame, List[str], Dict[str, str]]:
    """
    Returns a display DataFrame with rows=criteria (possibly MultiIndex by category),
    columns=candidate names, cells=icons (+optional numeric). Also returns the ordered
    list of criteria and the mapping criterion->category (for Evidence Explorer).
    """
    # numeric matrix: criteria x candidates
    ...
    # Build matrix
    mat = coverage_df.set_index("Candidate")[crit_cols].T  # rows=criteria, cols=candidates

    # categories
    cat_map = extract_categories_map(jd_text or "", crit_cols)
    ordered = sorted(crit_cols, key=lambda c: (cat_map.get(c, "zz"), c.lower()))
    # Build display df with strings
    disp = pd.DataFrame(index=pd.MultiIndex.from_tuples([(cat_map[c], c) for c in ordered], names=["Category","Criterion"]), columns=candidates)
    for c in candidates:
        for crit in ordered:
            val = float(mat.loc[crit, c])
            disp.loc[(cat_map[crit], crit), c] = build_icon(val, hi, lo, show_scores)
    return disp, ordered, cat_map
...
def styled_table(df: pd.DataFrame):
    ...
    try:
        ...
        # Light background bands per category if MultiIndex
        styler = df.style
        return styler
    except Exception:
        return df.style
...
def have_openai() -> bool:
    return openai_mod is not None and hasattr(openai_mod, "OpenAI")
...
def build_openai_client():
    ...
    try:
        ...
        if not api_key:
            return None, "OPENAI_API_KEY not set (.env)"
        client = OpenAI(api_key=api_key)
        return client, None
    except Exception as e:
        return None, str(e)

def gpt_insights_for_candidates(
    model: str,
    jd_text: str,
    coverage_df: pd.DataFrame,
    snippets_map: Dict[str, List[Tuple[str, float]]],
    redacted: bool = False,
) -> Dict[str, Dict[str, Any]]:
    client, err = build_openai_client()
    if client is None:
        st.warning(f"OpenAI client not available: {err}")
        return {}
    ...
    with st.spinner("Generating insights with GPT…"):
        ...
        for i, name in enumerate(names):
            row = coverage_df[coverage_df["Candidate"] == name].iloc[0].to_dict()
            overall = float(row.get("Overall", 0.0))
            crit_pairs = [(k, float(v)) for k, v in row.items() if k not in ("Candidate", "Overall")]
            crit_pairs.sort(key=lambda x: -x[1])
            snips = snippets_map.get(name, [])[:5]
            safe_snips = [{"snippet": (_redact_pii(s) if redacted else s)[:800], "score": float(sc)} for s, sc in snips]
            jd_excerpt = (jd_text or "")
            if redacted: jd_excerpt = _redact_pii(jd_excerpt)
            jd_excerpt = "\n".join(jd_excerpt.splitlines()[:40])

            sys = ("You are an expert technical recruiter. "
                   "Given a job description excerpt, criterion scores for a candidate, and short snippets, "
                   "return concise JSON with keys: top (3-5 bullets), gaps (2-4 bullets), notes (one sentence). "
                   "Keep bullets brief (~16 words).")
            user_payload = {
                "job_description_excerpt": jd_excerpt,
                "candidate_name": name,
                "overall_score": overall,
                "criteria": [{"name": k, "score": v} for k, v in crit_pairs],
                "snippets": safe_snips,
                "output_schema": {"top": ["..."], "gaps": ["..."], "notes": "..."}
            }
            try:
                ...
                if isinstance(data, dict):
                    top = [normalize_ws(x) for x in data.get("top", []) if normalize_ws(str(x))]
                    gaps = [normalize_ws(x) for x in data.get("gaps", []) if normalize_ws(str(x))]
                    notes = normalize_ws(str(data.get("notes", "")))
                else:
                    top, gaps, notes = [], [], text[:400]
                results[name] = {"top": top[:5], "gaps": gaps[:4], "notes": notes}
            except Exception as e:
                results[name] = {"top": [], "gaps": [], "notes": f"LLM error: {e}"}
            prog.progress((i + 1) / total, text=f"Analyzing {name}…")
        try: prog.empty()
        except Exception: pass
    return results

# --------- Core analysis (local scoring) ---------
@st.cache_data(show_spinner=False)
...
st.caption("Local scoring with optional GPT-4o insights. Silent embedding; scoring bar only.")
...
with st.sidebar:
    ...
    if colb2.button("Reset session"):
        for k in ["cached_candidates","last_coverage","last_insights","last_snippets","jd","jd_manual_text","criteria_text","weights_csv","evidence_map"]:
...
if st.session_state.get("_trigger_analyze", False):
    # Criteria source: prefer jd-derived suggestions (user can manage elsewhere)
    def get_active_criteria() -> List[str]:
        txt = st.session_state.get("criteria_text", "").strip()
        if txt:
            items = [normalize_ws(x) for x in txt.splitlines() if normalize_ws(x)]
            return items
        jd = st.session_state.get("jd")
        if jd and getattr(jd, "text", ""):
            sug = extract_requirements_from_jd(jd.text)
            if sug:
                chosen = sug[:10]
    ...
    if not criteria:
        st.warning("No criteria to analyze. Add criteria (Job Description tab) then click Analyze.")
    elif not st.session_state.cached_candidates:
        ...
        with st.spinner("Running analysis…"):
            ...
            if isinstance(cov, pd.DataFrame) and not cov.empty:
                if engine in ("gpt-4o-mini", "gpt-4o"):
                    ...
                    gpt_map = gpt_insights_for_candidates(engine, (st.session_state.jd.text if st.session_state.get('jd') else ""), cov, snips, redacted=redacted)
                    st.session_state.last_insights = gpt_map if gpt_map else ins_local
                else:
...
# Tabs
overview_tab, jd_tab, coverage_tab, insights_tab, compare_tab, export_tab, settings_tab = st.tabs(
    ["Overview","Job Description","Coverage","Insights","Compare","Export","Settings"]
)
...
with jd_tab:
    ...
    if st.session_state.get("jd"):
        sug = extract_requirements_from_jd(st.session_state.jd.text)
        if sug:
            pick = st.multiselect("Add/remove items then click 'Apply to Criteria'", options=sug, default=sug[:10], key="jd_sug_pick")
            if st.button("Apply to Criteria"):
                st.session_state.criteria_text = "\n".join(pick)
                st.success("Updated criteria from JD suggestions.")
        else:
            st.info("No bullet-like lines detected. You can still paste criteria manually in the sidebar.")
...
with coverage_tab:
    ...
    if not isinstance(coverage_df, pd.DataFrame) or coverage_df.empty:
        ...
        # Controls for thresholds and score display
        ...
        disp, ordered_criteria, cat_map = build_coverage_matrix_view(coverage_df, jd_text, show_scores, hi, lo)
        ...
        names = coverage_df["Candidate"].tolist()
        crit = st.selectbox("Criterion", ordered_criteria, index=0 if ordered_criteria else None)
        cand_choice = st.selectbox("Candidate", names, index=0 if names else None)
        ev = st.session_state.get("evidence_map", {})
        if crit and cand_choice and ev:
            snip, sc = ev.get((cand_choice, crit), ("(no snippet found)", 0.0))
            st.markdown(f"**{cand_choice} → {crit}**  — score: `{sc:.2f}`")
            st.code(snip)

with insights_tab:
    st.subheader("Candidate Insights & Snippets")
    insights_map = st.session_state.get("last_insights", {})
    snippets_map = st.session_state.get("last_snippets", {})
    covdf = st.session_state.get("last_coverage", pd.DataFrame())
    ...
    if not insights_map or (isinstance(covdf, pd.DataFrame) and covdf.empty):
        ...
        for name in order_names:
            ...
            with st.expander(f"🔎 {name}"):
                ...
                sn = snippets_map.get(name, [])
                if sn:
                    st.markdown("**Best-matching snippets**")
                    for s, sc in sn:
                        st.write(f"Score: {sc:.2f}"); st.code(s)
...
with compare_tab:
    ...
    if not names:
        ...
        if len(sel) < 2:
            ...
            st.markdown("**Per-criterion view**")
            crit_cols = [c for c in sub.columns if c != "Overall"]
            for crit in crit_cols:
                st.markdown(f"### {crit}")
                rows = [{"Candidate": nm, "Score": float(sub.loc[nm, crit])} for nm in sel]
                chart_df = pd.DataFrame(rows)
                try:
                    import altair as alt
                    ch = alt.Chart(chart_df).mark_bar().encode(x="Candidate:N", y="Score:Q", tooltip=["Candidate","Score"]).properties(height=140)
                    st.altair_chart(ch)
                except Exception:
...
with export_tab:
    ...
    if not isinstance(covdf, pd.DataFrame) or covdf.empty:
        ...
        jd_text = st.session_state.jd.text if st.session_state.get("jd") else ""
        # Use numeric matrix for export (not icon view)
...
with settings_tab:
    ...
    with st.popover("🔒 Privacy & data handling"):
        st.markdown(
            "> **When using GPT-4o or GPT-4o-mini**, data from Job Descriptions and Candidate Resumes is **securely transmitted** to OpenAI’s servers via their API (application programming interface)."
        )
        st.markdown(
            "- ✅ **API data is *not* used to train OpenAI models** (unless you explicitly “opt in” in your OpenAI account settings).\n"
            "  We have **not opted in** for our API key.\n"
            "  According to OpenAI, your uploaded Job Descriptions and Candidate Resumes are **not used to train models**, and are **not made public or shared**."
        )
        st.markdown(
            "- ✅ **API data is retained for up to 30 days** for abuse monitoring, then deleted automatically."
        )
        st.markdown(
            "- ❗ If this level of data handling is unacceptable, please use **Local** mode or enable **Redacted mode**."
        )
