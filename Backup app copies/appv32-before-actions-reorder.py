...
with st.sidebar:
    ...
    pages = ["Upload & Analyse", "Criteria from JD", "Candidate Scores", "Candidate Insights", "Compare", "Candidates", "Scoring Explained", "Export", "PDF Tools", "Settings"]
...
if current_page == "Upload & Analyse":
        ...
        with result_col1:
            ...
            try:
                ...
                for idx, r in top3.iterrows():
                    st.write(f"**{idx+1}.** {r['Candidate']} — Score: {r['Overall']:.2f}")
            except Exception:
                pass
        
        with result_col2:
            # Heading with inline help icon - using columns for better control
            heading_col, help_col = st.columns([0.95, 0.05])
            with heading_col:
                st.markdown("### 📈 Top Matched Criteria")
            with help_col:
                st.markdown(
                    '<div style="margin-top: 0.5rem;">'
                    '<span style="color: #888; cursor: help; font-size: 1rem; user-select: none;" '
                    'title="Here are the evaluation criteria, extracted from your job description, '
                    'that scored the highest average match/similarity with candidate resumes.">'
                    'ℹ️</span></div>',
                    unsafe_allow_html=True
                )
            try:
                ...
                for crit, score in top_5:
                    st.write(f"• {crit}: Score: {score:.2f}")
            except Exception:
                pass
    ...
    if st.session_state.get("jd"):
        ...
        with col1:
            st.metric("JD File", jd_obj.file_name.split('.')[0][:15] + "..." if len(jd_obj.file_name) > 18 else jd_obj.file_name)
        with col2:
        ...
        # JD Preview (collapsed by default)
        with st.expander("📄 View full text extracted from JD file"):
            st.text_area("JD content", value=jd_obj.text, height=300, label_visibility="collapsed")
    else:
    ...
    st.caption("GPT automatically extracts criteria from your job description into organized categories.")
    ...
    if secs_dict:
        with st.expander("🔍 View extracted sections (auto from GPT)", expanded=False):
    ...
    with save_col2:
        st.info("💡 **Tip:** Add, remove, or reword criteria. Uncheck **Use** to exclude items. Changes take effect after clicking Save.", icon="ℹ️")

elif current_page == "Candidate Scores":
    st.markdown("<div style='margin-top: 1.5rem;'></div>", unsafe_allow_html=True)
    ...
    with header_col1:
        ...
        if not isinstance(coverage_df, pd.DataFrame) or coverage_df.empty:
            ...
            # Info box with scoring guide
            st.info("**Scoring:** ✅ = Strong match | ⚠️ = Moderate | ⛔ = Weak or missing.\n\nSee **Scoring Explained** tab for details.", icon="ℹ️")
