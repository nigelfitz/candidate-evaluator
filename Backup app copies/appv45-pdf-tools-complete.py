...
with st.sidebar:
    ...
    pages = ["Upload & Analyse", "Job Criteria", "Scoring Analysis", "Candidate Insights", "Export Reports", "PDF Tools", "Settings", "Help & FAQ"]
...
""")
    ...
    with st.expander("🔧 Troubleshooting", expanded=False):
        ...
        st.markdown("#### PDF text extraction problems")
        st.markdown("""
If PDFs are extracting with text in the wrong order or appearing jumbled:
""")
...
- If a PDF is an image scan, use OCR software first to convert to text
...
""")
    ...
    **Reminder:** When Nigel says “Let's finish the PDF Tools page based on the notes on that page,”
    ...
    if not tesseract_ok:
        st.error("⚠️ Tesseract OCR not detected. Install Tesseract and ensure it's in your system PATH to enable OCR.", icon="🚫")
        st.stop()
    ...
    with st.expander("ℹ️ System Information"):
        ...
        if p2i_ok:
            st.caption("Note: pdf2image requires Poppler binaries. If not available, PyMuPDF rasterization will be used automatically.")
        ...
        for uf in conv_files:
            ...
            with st.spinner("Processing PDF..."):
                try:
                    ...
                    # Pass 2: Try OCR if text is minimal
                    if initial_length < 200:
                        ...
                        # Try pdf2image first, but fall back to PyMuPDF if Poppler isn't available
                        used_backend = None
                        pages = None
                        ...
                        if pdf2image is not None:
                            try:
                                ...
                                if "poppler" in str(pdf2img_err).lower():
                                    pages = None  # Will trigger PyMuPDF fallback
                                else:
                                    raise
                        
                        # Fallback to PyMuPDF rasterization if pdf2image failed or unavailable
                        ...
                        if pages:
                            ...
                            if text_ocr:
                                ...
                                st.warning("OCR completed but no text was extracted. The PDF may be blank or the image quality too poor.")
                                continue
                        else:
