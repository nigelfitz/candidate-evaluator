...
def check_password():
    ...
    def password_entered():
        ...
        if st.session_state["password"] == st.secrets.get("password", "testpass123"):
...
# Load dotenv first (lightweight and needed early)
...
# Critical imports (needed for basic functionality)
try:
    from openai import OpenAI  # type: ignore
except Exception:
    OpenAI = None  # type: ignore

fitz = _try_import("fitz")  # PyMuPDF - needed for PDF processing

# Deferred imports (only import when first used)
pdfplumber = None
docx = None
sklearn = None
reportlab = None
...
def _ensure_reportlab():
    ...
    if reportlab is None:
        ...
        if reportlab:
            ...
            globals()['A4'] = A4
            globals()['rl_canvas'] = rl_canvas
            globals()['cm'] = cm

def _ensure_file_readers():
    global pdfplumber, docx, openpyxl
...
if current_page == "Upload & Analyse":
    ...
    with col1:
        ...
        jd_uploader_key = st.session_state.get("jd_uploader_key", 0)
        jd_file = st.file_uploader("Upload JD", type=["pdf","docx","txt"], key=f"jd_upl_{jd_uploader_key}", accept_multiple_files=False, label_visibility="collapsed")
    ...
    with col2:
        ...
        uploaded = st.file_uploader("Upload resumes", accept_multiple_files=True, type=["pdf","docx","txt"], key=f"cand_upl_{uploader_key}", label_visibility="collapsed")
    ...
    if jd_file is not None:
        ...
        # Clear the JD uploader
        st.session_state.jd_uploader_key = st.session_state.get("jd_uploader_key", 0) + 1
        st.rerun()
    elif jd_manual.strip():
        # Only update if manual text changed
        if not st.session_state.get("jd") or st.session_state.jd.text != jd_manual:
            st.session_state.jd = JD(file_name="Manual JD", text=jd_manual, hash=hash_text(jd_manual), raw_bytes=None)

    # Candidates ingest
    new_candidates: List[Candidate] = []
    if uploaded:
        # Track which files from the uploader are already in cached_candidates
        existing_hashes = {c.hash for c in st.session_state.get("cached_candidates", [])}
        
        for uf in uploaded:
            b = uf.read()
            file_hash = hash_bytes(b)
            ...
            new_candidates.append(Candidate(name=name_guess, file_name=uf.name, text=text, hash=file_hash, raw_bytes=b))
    
    if new_candidates:
        # Add new candidates to existing ones
        all_candidates = st.session_state.get("cached_candidates", []) + new_candidates
        
        # Make candidate names unique by appending file-based suffix if duplicates exist
        name_counts = {}
        for cand in all_candidates:
            base_name = cand.name
            if base_name in name_counts:
                name_counts[base_name] += 1
                # Append a short file identifier to make it unique
                file_suffix = cand.file_name.replace('.pdf', '').replace('.docx', '').replace('.txt', '')[-15:]
                cand.name = f"{base_name} ({file_suffix})"
            else:
                name_counts[base_name] = 1
        
        st.session_state.cached_candidates = all_candidates
        
        # Clear the uploader by incrementing its key (prevents duplicate uploads)
                ...
                for idx, cand in enumerate(current_candidates):
                    ...
                    with col_btn:
                        if st.button("❌", key=f"remove_cand_{idx}", help=f"Remove {cand.name}"):
                            ...
                            st.session_state.last_coverage = pd.DataFrame()
                            st.session_state.last_insights = {}
                            # Increment uploader key to force it to reset (clears the widget)
                            st.session_state.uploader_key = st.session_state.get("uploader_key", 0) + 1
                            st.rerun()
    
    # === GPT Insights Selection ===
    if current_jd and current_candidates:
        st.markdown("---")
        st.markdown("### 🤖 GPT Insights Options")
        st.caption("Control which candidates receive detailed AI-powered insights (affects cost and processing time)")
        ...
        with col1:
            insight_mode = st.radio(
                "Select candidates:",
                ["Top N candidates", "All candidates", "Custom selection"],
                key="insight_mode",
                horizontal=True
            )
        
        if insight_mode == "Top N candidates":
            with col2:
                top_n = st.number_input(
                    "How many:",
                    min_value=1,
                    max_value=min(50, len(current_candidates)),
                    value=min(5, len(current_candidates)),
                    key="insights_n_input"
                )
            st.session_state.insights_top_n = top_n
            st.session_state.insights_mode = "top_n"
            st.session_state.insights_custom = []
                
        elif insight_mode == "All candidates":
            st.session_state.insights_mode = "all"
            st.session_state.insights_top_n = len(current_candidates)
            st.session_state.insights_custom = []
            st.info(f"Will generate insights for all {len(current_candidates)} candidates")
            
        else:  # Custom selection
            candidate_names = [c.name for c in current_candidates]
            selected = st.multiselect(
                "Select specific candidates:",
                candidate_names,
                default=candidate_names[:min(5, len(candidate_names))]
            )
            st.session_state.insights_mode = "custom"
            st.session_state.insights_custom = selected
            st.session_state.insights_top_n = len(selected)
    ...
    # ===== Update Sidebar Cost Estimate =====
    # This runs after file processing so cost appears immediately
    with st.sidebar:
        current_jd = st.session_state.get("jd")
        current_candidates = st.session_state.get("cached_candidates", [])
        ...
        if current_jd and current_candidates:
            ...
            # Get number of candidates for insights (default to 5 or total if less)
            num_insights = st.session_state.get("insights_top_n", min(5, len(current_candidates)))
            
            # Calculate cost based on selected number for insights
            jd_text = current_jd.text
            # Only calculate cost for candidates getting insights
            all_candidate_texts = [c.text for c in current_candidates]
            selected_candidate_texts = all_candidate_texts[:num_insights]  # Rough approximation
            
            cost_info = estimate_analysis_cost(jd_text, selected_candidate_texts, num_criteria)
            
            # Display breakdown format
            st.markdown(f"**Estimated Total: ${cost_info['estimated_cost_usd']:.2f}**")
            st.caption(f"Insights: ${cost_info['breakdown']['candidate_insights']:.2f} for {num_insights} candidates")
            st.caption(f"JD criteria: ${cost_info['breakdown']['jd_extraction']:.2f}")
            st.caption("Costs are explained in FAQ's")
    ...
    for crit in ordered:
        ...
        scores_for_crit = []
        for cand in candidates:
            val = mat.loc[crit, cand]
            # Handle case where mat.loc returns a Series (duplicate names in index)
            if isinstance(val, pd.Series):
                val = val.iloc[0]
            scores_for_crit.append(float(val))
    ...
    # Ensure export libraries are loaded for downloads
    _ensure_reportlab()
    _ensure_file_readers()
    ...
    # Get list of candidates who received GPT insights
    gpt_insights_candidates = st.session_state.get("gpt_insights_candidates", [])
    
    # Filter to only show candidates with GPT insights
    if not gpt_insights_candidates:
        st.info("No GPT insights generated yet. Run analysis with 'Generate GPT Insights' enabled.")
        st.stop()
    ...
    # Filter coverage dataframe to only include candidates with GPT insights
    covdf_filtered = covdf[covdf["Candidate"].isin(gpt_insights_candidates)].copy()
    
    # Sort candidates by Overall score (descending) and add rank
    covdf_sorted = covdf_filtered.sort_values("Overall", ascending=False).reset_index(drop=True)
    ...
    # Check if this candidate has GPT insights (not just baseline)
    has_gpt_insights = bool(info.get("top") or info.get("gaps") or info.get("notes"))
    
    if not has_gpt_insights:
        st.warning(f"""
        **⚠️ GPT insights not yet generated for {selected_name}**
        
        This candidate was scored but didn't receive detailed AI analysis. This happens when:
        - You selected "Top N" and this candidate wasn't in the top group
        - You used custom selection and didn't include this candidate
        
        **To generate insights for this candidate:** Go to Upload & Analyse, select this candidate 
        for insights, and run analysis again.
        """)
        
        st.info(f"""
        **Scoring data is available:**
        - Overall score: **{selected_score:.2f}** (Rank #{selected_rank})
        - See "Detailed Scores" below for criterion-by-criterion breakdown
        """)
    else:
        # Show GPT insights normally
...
""")
    ...
    with st.expander("💬 Frequently Asked Questions", expanded=False):
        st.markdown("### Why is there a fee for running each analysis?")
        st.markdown("""
This application uses **OpenAI's GPT-4o** (a large language model) to:
- Extract structured criteria from job descriptions
- Generate detailed insights about each candidate
- Identify strengths, gaps, and provide contextual recommendations

**What OpenAI charges for:**
- **Input tokens:** The text we send (job description + resume content)
- **Output tokens:** The AI-generated analysis it returns

Each analysis typically costs **$0.01-0.02 per candidate** depending on resume length and number of criteria. 
The cost estimate shown before you run an analysis reflects these API charges.

**Note:** Scoring (semantic similarity) is free and runs locally - only the GPT insights require API calls.
""")
...
""")
        ...
        st.markdown("### Can I edit criteria?")
        st.markdown("""
**Yes!**
...
""")
    ...
    # PDF Preview Section (full width, below the buttons)
    if reportlab is not None:
        with st.expander("🔍 Preview Executive Summary PDF", expanded=False):
            jd_filename = st.session_state.jd.file_name if st.session_state.get("jd") else "Job Description"
    ...
    if not criteria:
        ...
        with st.spinner("Running analysis…"):
            ...
            if api_ok and isinstance(cov, pd.DataFrame) and not cov.empty:
                # Determine which candidates should get GPT insights
                insights_mode = st.session_state.get("insights_mode", "top_n")
                insights_top_n = st.session_state.get("insights_top_n", 5)
                insights_custom = st.session_state.get("insights_custom", [])
                
                # Get list of candidates to process
                if insights_mode == "all":
                    candidates_for_insights = cov['Candidate'].tolist()
                elif insights_mode == "custom":
                    candidates_for_insights = insights_custom
                else:  # top_n
                    # Sort by overall score and take top N
                    sorted_cov = cov.sort_values(by="Overall", ascending=False)
                    candidates_for_insights = sorted_cov.head(insights_top_n)['Candidate'].tolist()
                
                num_insights = len(candidates_for_insights)
                total_candidates = len(cov)
                
                status_container.info(f"✨ Generating GPT insights for {num_insights} of {total_candidates} candidates...")
                
                try:
                    with st.spinner(f"Generating GPT insights for {num_insights} candidates…"):
                        jd_text_full = st.session_state.jd.text if st.session_state.get("jd") else ""
                        upgraded = st.session_state.get("last_insights", {}).copy()  # Keep existing insights
                        crit_cols = [c for c in cov.columns if c not in ("Candidate","Overall")]
                        
                        # Track which candidates get GPT insights
                        gpt_insights_generated = []
                        
                        for idx, row in cov.iterrows():
                            cand_name = row["Candidate"]
                            
                            # Only generate GPT insights for selected candidates
                            if cand_name in candidates_for_insights:
                                ...
                                upgraded[cand_name] = gpt_candidate_insights(
                                    ...
                                    evidence_map=st.session_state.get("evidence_map", {}),
                                ) or upgraded.get(cand_name, {})
                                
                                # Track that this candidate got GPT insights
                                gpt_insights_generated.append(cand_name)
                            # Else: keep existing local/baseline insight if it exists
                        
                        # Update insights (GPT for selected, baseline for others)
                        st.session_state["last_insights"] = upgraded
                        # Store which candidates got GPT insights
                        st.session_state["gpt_insights_candidates"] = gpt_insights_generated
                        status_container.success(f"✅ Analysis complete! Generated insights for {num_insights} candidates.")
                        try: 
                            st.toast(f"Insights generated for top {num_insights} candidates.", icon="✨")
                        except Exception: 
                            pass
                except Exception as e:
                    st.warning(f"GPT insight generation failed — using local insights. Error: {e}")
            ...
            try:
                ...
                # Get number of candidates that actually got GPT insights
                insights_mode = st.session_state.get("insights_mode", "top_n")
                insights_top_n = st.session_state.get("insights_top_n", 5)
                num_insights_generated = insights_top_n if api_ok else 0
                
                # Calculate cost based on actual insights generated
                all_candidate_texts = [c.text for c in st.session_state.cached_candidates]
                insights_candidate_texts = all_candidate_texts[:num_insights_generated]
                total_resume_chars = sum(len(t) for t in all_candidate_texts)
                
                cost_info = estimate_analysis_cost(jd_text, insights_candidate_texts, len(criteria))
